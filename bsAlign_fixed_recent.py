...
def deaminate(source,strand='ab'):
    ...
    print("Writing converted sequences to {}".format(suffix(source, strand)))
...
def run_BLAST(query,subject,XML,mask):
	...
	print([mbdb,'-dbtype','nucl','-parse_seqids','-in',subject])
        call([mbdb,'-dbtype','nucl','-parse_seqids','-in',subject])
        #print [mbdb,'-dbtype','nucl','-in',subject]
        #call([mbdb,'-dbtype','nucl','-in',subject])
